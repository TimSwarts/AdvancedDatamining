...
def scatter(xs, ys, *, model=None):
    ...
    if isinstance(xs, np.ndarray):
        back = [[model.predict(np.array([[x1, x2]])).tolist()[0] for x1 in xgrid] for x2 in xgrid]
...
def main():
    xs, ys = linear('nominal')
    print(f'xs({len(xs)} lists of attributes): {xs}')
    print(f'ys({len(ys)} nominal classification values): {ys}')
    print(f'value type of xs attributes: {type(xs[0][0])}')
    print(f'value type of ys: {type(ys[0])}')
    return 0
